...
def transform_geojson_task(
    ...
    for feature in features:
        ...
        if level_config.parent_level:
            # Assume parent code property follows pattern: parent_level.lower() + "_code"
            parent_code_property = level_config.parent_level.lower() + "_code"
            parent_code = properties.get(parent_code_property)
            # If not found, try alternative naming
            if parent_code is None:
                parent_code = properties.get(f"{level_config.parent_level.lower()}_id")
            if parent_code is None:
...
def lookup_parent_task(
    ...
    Returns:
        DataFrame with Parent column added (parent ID)
    """
    if reference_df.is_empty():
        # No parent level, add null Parent column
        return transformed_df.with_columns(
            [pl.lit(None).cast(pl.Int64).alias("Parent")]
        )

    # Join to get parent ID
    result = transformed_df.join(
        reference_df, left_on="ParentCode", right_on="code", how="left"
    ).select(["Name", "Code", "Geometry", "ParentCode", pl.col("Id").alias("Parent")])
...
def import_geojson_level_flow(
    ...
    if not final_df.is_empty():
        load_to_database_task(
            db_helper=db_helper,
            df=final_df.select(["Code", "Name", "Geometry", "Parent", "Type"]),
            table_name="Zone",
        )
    else:
