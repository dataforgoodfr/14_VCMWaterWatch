 ...
 */
export async function fetchMetaTables(
  ...
  try {
    ...
    const metaTablesResponse = await instance.get(
      `/meta/bases/${process.env.NOCODB_BASE_ID}/tables`
    );

    if (metaTablesResponse.status !== 200) {
      throw new Error(
        `Failed to fetch meta tables: ${metaTablesResponse.statusText}`
      );
    }
    ...
    tableCache = metaTablesData.list
      .filter((table): table is MetaTable & { table_name: string } =>
        Boolean(table.table_name)
      )
      .map((table) => ({
        id: table.id,
