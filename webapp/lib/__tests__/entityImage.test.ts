...
describe('getEntityImageSrc — country', () => {
	...
	it('returns the correct URL for a known country code', () => {
		process.env.EXPORT_IMAGES_DIR = tmpDir
...
describe('getEntityImageSrc — team', () => {
	it('returns the correct URL for a team member slug', () => {
		process.env.EXPORT_IMAGES_DIR = tmpDir
		writeManifest('team', { 'gaspard-lemaire': 'gaspard-lemaire.a1b2c3.jpg' })
		expect(getEntityImageSrc('team', 'gaspard-lemaire')).toBe('/images/team/gaspard-lemaire.a1b2c3.jpg')
	})

	it('returns null for an unknown team member', () => {
		process.env.EXPORT_IMAGES_DIR = tmpDir
		writeManifest('team', { 'alice-smith': 'alice-smith.abc.jpg' })
		expect(getEntityImageSrc('team', 'unknown-member')).toBeNull()
	})

	it('maintains independent cache per entity', () => {
		process.env.EXPORT_IMAGES_DIR = tmpDir
		writeManifest('country', { FR: 'FR.abc.jpg' })
		writeManifest('team', { 'alice-smith': 'alice-smith.abc.jpg' })

		expect(getEntityImageSrc('country', 'FR')).toBe('/images/country/FR.abc.jpg')
		expect(getEntityImageSrc('team', 'alice-smith')).toBe('/images/team/alice-smith.abc.jpg')
		// Cross-entity: country key should not match team manifest
