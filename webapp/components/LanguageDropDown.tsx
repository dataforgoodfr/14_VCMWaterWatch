...
export default function LanguageDropDown() {
  ...
  const onLanguageChange = (option: OptionProps) => {
    ...
    const locales = Object.keys(languagesOptionsTranslations || {}) as Array<
      keyof typeof languagesOptionsTranslations
    >;
    ...
    router.push(
      `/${option.value}${updatedPathname}?${newSearchParams.toString()}`
    );
  };

  const allLocales = Object.keys(languagesOptionsTranslations || {}) as Array<
    keyof typeof languagesOptionsTranslations
  >;
        ...
        <DropdownMenuRadioGroup>
          {otherLocales.map((otherLocale) => (
            <DropdownMenuRadioItem
              ...
              value={otherLocale}
              onClick={() =>
                onLanguageChange({ text: otherLocale, value: otherLocale })
              }
            >
