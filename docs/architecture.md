...
**Serving:** A dedicated Next.js route handler at `webapp/app/images/[entity]/[...path]/route.ts` serves requests to `/images/...` by reading files directly from `EXPORT_IMAGES_DIR` on the host filesystem. This mirrors the PMTiles pattern (`webapp/app/pmtiles/[...path]/route.ts`) so both assets share the same single code path in dev and prod, driven by env vars rather than Next.js' static file handler (which cannot serve files from a volume mounted outside `public/`). The handler sets `Cache-Control: public, max-age=31536000, immutable` (safe because filenames are content-hashed). The `/api/countries/[code]` BFF route resolves the mirrored URL server-side via `webapp/lib/entityImage.ts`, which reads `manifest.json` with mtime-based cache invalidation (no restart needed when the volume updates). The `CountryProfileDetail` client component receives the resolved URL as a prop and uses `priority` on the `<Image>`.

**Volume layout (prod):** `images-data` named volume mounted at `/public/images` on both `worker` (read/write) and `webapp` (read). `EXPORT_IMAGES_DIR=/public/images` is set on the webapp service in `docker-compose.deploy.yml`.

**Local dev:** Set `EXPORT_IMAGES_DIR=../data/export/images` in `webapp/.env.local` (already done). A seed `manifest.json` is committed at `data/export/images/`. Refresh the seed with `just pipelines export-country-images / export-team-images`.
...
External Sources (Eurostat, Ofwat, Vewin, ...)
    ...
    data/staging/ + staging.duckdb
      ...
      NocoDB  ◄──── Manual edits by volunteers
        ...
        │                       │                               │
        │                       ▼                               ▼
        │               Next.js /pmtiles/* ──▶ MapLibre          Next.js /images/*
...
- **Shared volumes:**
  ...
  - `images-data` — mounted into both `webapp` (read) and `worker` (read/write) at `/public/images`
